...
[vp, facedir, expdir, MonitorSelection, response_mapping, instruct] = get_experimentInfo(run_mode);
[TastenVector, ~, response_keys] = prepare_responseKeys(response_mapping); % manual responses
check_dir([expdir, vp, '\']); % check for (and possibly create) subject data directory
logfname = ['.\log\', vp, '_qlog.mat'];
stimfname = 'fm_struct_npic_470x349.mat';
...
[stim] = get_stims([facedir, stimfname]); % see subfunction
...
try
    ...
    Bubbles.Hz         = hz;
    Bubbles.mapping    = response_mapping;
    Bubbles.stmfile    = stimfname;
    
    save([expdir, vp, '\', outfilename], 'Bubbles');
    qlog.q = Bubbles.q;
    qlog.blocknum = blocknum;
    save(logfname, 'qlog');
...
if ~ (exist(subdir, 'dir') == 7)
    mkdir(subdir); end
tmp = dir; 
if ~ ismember('log', {tmp([tmp.isdir]).name})
     mkdir('log');
end
end


function [stim] = get_stims(stimFileName) 
    tmpname = load(stimFileName);
    stim = tmpname.struct_npic;
    % make one block 60 trials
    stim.npic   = repmat(stim.npic, 3, 1);
    stim.names  = repmat(stim.names, 3, 1);
    stim.cond   = repmat(stim.cond, 3,1);
end
...
function [vp, facedir, expdir, MonSelection, response_mapping, instruct] = get_experimentInfo(run_mode)
if run_mode < 3
  vp = input('\nParticipant (three characters, e.g. S01)? ', 's');
    if length(vp)~=3 
       error ('Wrong input!'); end
  response_mapping = str2num(input('\nResponse mapping?\n1: y = weiblich, m = männlich\n2: y = männlich, m = weiblich\n', 's'));    
      if ~ismember(response_mapping, [1, 2])
        error('\nWrong mapping code!'); end
else
      vp = 'tmp';
      response_mapping = 1;
end
...
if strcmp(getenv('COMPUTERNAME') , 'DESKTOP-MDU4B4V')
  facedir  = 'C:\Users\Gregor\Filr\Meine Dateien\Bubbles_Faces\';
  expdir   = 'C:\Users\Gregor\Filr\Meine Dateien\Bubbles_Faces\data\';
  MonSelection = 4;
elseif strcmp(getenv('COMPUTERNAME') , 'PC2022001273')
  ...
  MonSelection = 5;
elseif strcmp(getenv('COMPUTERNAME') , 'PC1012101290')
  facedir  = 'D:\Bubbles_Faces\';
  expdir   = 'D:\Bubbles_Faces\data\';
  MonSelection = 3;
end
