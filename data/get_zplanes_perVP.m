...
% pre-allocate
zci    = zeros([numel(vpsel), dims]);

for nscale = 1:5%size(zci, 1)
    for vp = 1:numel(flist)
      ld = load(flist{vp});
      tmpp = ld.perm(nscale).perm_zci;
      tmpci(vp, nscale, :, :) = imfilter(tmpp, patches{nscale}/sum(patches{nscale}(:)), 'same');    % normalize filter output 
    end
